        ...
        const merged = (...args) => {
            ...
            if (errsExisting && errsExisting.length > 0) {
                errsExisting.forEach(errExisting => {
                    ...
                    Cypress.Allure.step(errMsg, allure_types_1.Status.FAILED, { message: errMsg, trace: errExisting.stack });
                });
            }
        };
...
const allureInterface = (env, fn) => {
    return {
        ...
        addEnvironmentInfo: (info) => fn({ task: 'addEnvironmentInfo', arg: { info } }),
        ...
        startStep: (name, date) => {
            ...
            return fn({ task: 'stepStarted', arg: { name, date: date !== null && date !== void 0 ? date : Date.now() } });
        },
        // remove from interface
        mergeStepMaybe: (name) => fn({ task: 'mergeStepMaybe', arg: { name } }),
        endStep: (status, statusDetails, date) => {
            // console.log('END STEP');
            return fn({
                task: 'stepEnded',
                arg: { status: status !== null && status !== void 0 ? status : allure_types_1.Status.PASSED, details: statusDetails, date: date !== null && date !== void 0 ? date : Date.now() },
            });
        },
        step: (name, status, details) => fn({ task: 'step', arg: { name, status: status !== null && status !== void 0 ? status : allure_types_1.Status.PASSED, date: Date.now(), details } }),
        ...
        tms: (url, name) => {
            const type = 'tms';
            const fullUrl = (0, common_1.tmsIssueUrl)(env, url, type);
            const linkName = name !== null && name !== void 0 ? name : url;
            return fn({ task: 'link', arg: { url: fullUrl, name: linkName, type } });
        },
        tmsWithId: (url, name) => {
            const type = 'tms';
            const fullUrl = (0, common_1.tmsIssueUrl)(env, url, type);
            const linkName = (0, common_1.descriptionId)(env, url, type, name);
            return fn({ task: 'link', arg: { url: fullUrl, name: linkName, type } });
        },
        issue: (url, name) => {
            const type = 'issue';
...
const isAfterEachHook = (test) => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    return test.type === 'hook' && test.hookName === 'after each';
};
const isHook = (test) => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    return test.type === 'hook';
};
const createTestsForFailedBeforeHook = (runner, test) => {
    var _a;
    let index = 0;
        ...
        if (ts) {
            if (index === 1) {
                ...
                if (ts.err) {
                    ...
                    ts.err.message =
                        `${ts.err.message}\n\n` +
                            `Because this error occurred during a \`before all\` hook we are skipping the remaining tests in the current suite: \`${(_a = ts.parent) === null || _a === void 0 ? void 0 : _a.title}\` (added by ${common_1.packageLog})`;
                }
            }
 ...
 */
const createTestsBeforeAfterEach = (runner, test) => {
    var _a;
    let index = 0;
    let failedIndex = 0;
    let found = false;
...
const registerMochaReporter = (ws) => {
    ...
    (0, cypress_events_1.handleCyLogEvents)(runner, allureEventsEmitter, {
        ...
        spyOnRequests: () => {
            var _a, _b;
            return (_b = (_a = Cypress.env('allureAddBodiesToRequests')) === null || _a === void 0 ? void 0 : _a.split(',')) !== null && _b !== void 0 ? _b : [];
        },
    });
};
